 ...
 * dml.c
 *
 * It provides access controls on DML statements
 *
...
/*
 * check_one_relation_privileges
 *
 * It actually checks required permissions on a certain relation
 * and its columns.
 */
static bool
check_one_relation_privileges(Oid relOid,
							  Bitmapset *selected,
							  Bitmapset *modified,
							  uint32 required,
							  bool abort)
...
sepgsql_dml_privileges(List *rangeTabls, bool abort)
{
	...
	foreach (lr, rangeTabls)
	{
		...
		foreach (li, tableIds)
		{
			 ...
			 */
			if (!check_one_relation_privileges(tableOid,
											   selectedCols,
											   modifiedCols,
											   required, abort))
				return false;
